metadata = {
    'protocolName': 'Save supernatent from magnetic 96 well plate',
...
def run(ctx):
    ...
    destination_plate = ctx.load_labware("biorad_96_wellplate_200ul_pcr", 5)
    ...
    for well_oi in wells_to_aspirate:
        ...
        pipette_oi.aspirate(200, aspirate_plate.wells(well_oi))  
        ctx.delay(seconds=0.1)
        pipette_oi.dispense(200, destination_plate.wells(well_oi))
        ctx.delay(seconds=0.1)
